...
PNMImage* reduceImageWidth(const PNMImage* image, size_t k) {
    PNMImage* reduced_image = (PNMImage*) image;
    ...
    for (size_t iteration = 0; iteration < k; iteration++) {
        ...
        double* new_energy_tab = malloc(height * new_width * sizeof(size_t));
        ...
        for (size_t a = 0; a < height; a++) {
            ...
            for (size_t b = 0; b < width; b++) {
                ...
                new_energy_tab[a * new_width + x] =
                        energy_tab[a * width + b];

                new_image->data[a * new_width + x] =
                        reduced_image->data[a * width + b];
        ...
        if(reduced_image != image)
            freePNM(reduced_image);
        free(energy_tab);
        free(min_path);
